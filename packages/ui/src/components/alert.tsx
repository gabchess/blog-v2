...
function AlertDescription({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'text-muted-foreground [&_a]:underline-offset-3 [&_a]:hover:text-foreground text-balance text-xs/relaxed md:text-pretty [&_a]:underline [&_p:not(:last-child)]:mb-4',
        className,
      )}
      {...props}
    />
...
function AlertAction({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('absolute right-2 top-1.5', className)}
      {...props}
    />
  );
}
